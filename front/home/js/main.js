...
//获取所有菜单
$.post(getMenuUrl,{uid:1}).then(res=>{
    if(res.code == '200'){
        data = res.data;
        data.push({ id: 9999, name: "回收站", pid: 0,istrash:true });
        data.unshift( {id:8888,name:'图片',pid:0,isImage:true} );
    ...
    bootbox.prompt({
        ...
        callback: function(result){
            ...
            if(result == ''){
                ...
                $.post(addMenuUrl,{"uid":1, "pid":curCatId, "name":result, "orderid":0}).then(res=>{
                    if(res.code==200){
...
function fnEditItem(name) {
    $(event.target).hide().siblings('.item-cont').text('').html(`<input class="form-control editinput" value="${name}" type="text" onblur="fnComplete()" /><!--<button onclick="fnComplete()">完成</button>-->`)
    $('.editinput').focus();
}
function fnComplete() {
    ...
    $.post(setItemUrl,{itemid:id,content:val}).then(res=>{
        if(res.code==200){
...
function renderItems() {
    //请求该分类下的item列表
    $.post(getCatyitemUrl,{mid:curCatId}).then(res=>{
        items = res.data.item;
        var str = '';
        items.forEach(item=>str+=`<div  class="item" data-id="${item.id}"><span data-id="${item.id}" draggable="true" ondragstart="fnDragStart(event)"  class="item-cont">${item.content}</span><span class="edit-item iconfont icon-edit1" data-item="${item}" onclick="fnEditItem('${item.content}')"></span></div>`);
        ...
        if(catArr.length){
            ...
            catArr.forEach(function (cat) {
                return str+=`<li class="menuItem" draggable="true" ondragstart="fnDragStartCat(event)" ondrop="fnDrop(event)" ondragover="fnDragOver(event)" ondragenter="fnDragEnter(event)" ondragleave="fnDragLeave(event)"  data-id="${cat['id']}" data-pid="${data['pid']}"><span class="iconfont icon-folder"></span>${cat.name}</li>`
            })
            str += "</ul>";
        }
        $('.itemList').html(str);
...
function renderImages() {
    var str = '';
    images.forEach(item=>str+=`<div draggable="true" ondragstart="fnDragStartImage(event)"  data-id="${item.id}"  class="item-image"><img   data-id="${item.id}" src="${item.attachment}" alt=""></div>`);
...
function fnDrop(ev) {
    ...
    if(istrash(event.target)){
        ...
        if(isCat){
            //删除分类
            $.post(deleteMenuUrl,{mid:sourceCatId}).then(res=>{
                if(res.code == 200){
            ...
            //删除item
            $.post(deleteItemUrl,{itemid:sourceItemId}).then(res=>{
                if(res.code == 200){
            ...
            //删除图片
            $.post(deleteImagesUrl,{imageid:sourceItemId}).then(res=>{
                if(res.code == 200){
        ...
        if(isItem){
            //归类item到某个分类下
            $.post(mvItemUrl,{itemid:sourceItemId,mid:targetCatId}).then(res=>{
                if(res.code==200){
...
function fnAdd(self) {
    if(!curCatId || curCatId== 8888 || curCatId == 9999){
        bootbox.alert('请选择一个非图片的类目');return;
    }
    if($('.newItemInput').val()==''){
        bootbox.alert('请输入item内容');return;
    }
    $.post(addItemUrl,{mid:curCatId,uid:1,content:$('.newItemInput').val()}).then(res=>{
        if(res.code==200){
